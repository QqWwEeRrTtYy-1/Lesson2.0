...
Score = 0
Score_1 = 0
test = Text(text = "Score: 0", x = -0.82, y = .4, scale = 1, origin = (0,0))
test_1 = Text(text = "Score: 0", x = +0.82, y = .4, scale = 1, origin = (0,0))
...
floor = Entity(model='quad', y=-.5, origin_y=.5, collider='box', scale=(2,10), visible=True, color = color.red)
...
def update():
    ...
    if hit_info.entity == right_wall:
        global Score
        Score += 1
        test.text = "Score: " + str(Score)
        reset()
    elif hit_info.entity == left_wall:
        global Score_1
        Score_1 += 1
        test_1.text = "Score: " + str(Score_1)
        reset()
