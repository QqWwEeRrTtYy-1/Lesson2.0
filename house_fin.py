...
def draw_street(x: int, y: int, width: int ,height: int):
    for i in range(4):
        z = x
        for i in range(5):
            def draw_house(x: int, y: int, height: int, width: int):
                ...
                road_width = width + width * 1.2
                road_line_color = "#333333"
                road_fill_color = "#333333"
                # считаем параметры
                ...
                def draw_road():
                    t.goto(x, y - height / 2)
                    t.pendown()
                    draw_rectangle(0, 0, road_width, road_height, road_line_color, road_fill_color)
            ...
            draw_house(x, y, 100, 200)
            x = x + width + 40

        x = z
        y = y + height + 70
draw_street(-500, - 300 , 200, 100)
t.mainloop()