...
while user_choise not in (1, 2, 3):
    ...
    inventory = int(input("""
        Илья Муромец собрался в дорогу, что он решил с собой взять?
        1 - Булаву
        2 - Меч
        3 - Копьё
        """))
...
while user_choise not in (1, 2, 3) and level == 3:
    ...
    user_choise = int(input("ваш ответ: "))
    if user_choise == 1:
        answer = int(input("Вы захотели воды, открыть рюкзак?"))
         if answer = "Да":
             print("У вас там лежит", inv)
        else:
            print("Едем дальше...")
    elif user_choise == 2:
        print("32")
